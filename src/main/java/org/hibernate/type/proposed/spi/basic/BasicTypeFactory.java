 ...
 */
package org.hibernate.type.proposed.spi.basic;
...
import org.hibernate.type.proposed.spi.BasicType;
...
import org.hibernate.type.proposed.spi.descriptor.java.JdbcRecommendedSqlTypeMappingContext;
...
public class BasicTypeFactory {
	...
	private final Map<RegistryKey,BasicType> registry = new HashMap<>();
	...
	public <T> BasicType<T> resolveBasicType(
		...
		BasicType impl = registry.get( key );
		if ( impl == null ) {
	...
	@SuppressWarnings("unchecked")
	private  <T> BasicType<T> resolveBasicType(
			JavaTypeDescriptor<T> javaTypeDescriptor,
			SqlTypeDescriptor sqlTypeDescriptor,
		...
		final BasicType existing = registry.get( key );
		if ( existing != null ) {
			return existing;
		}
